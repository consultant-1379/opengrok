...
if os.environ.get("OPENGROK_ROOT"):  # debug only
    ...
    OPENGROK_BASE_DIR = os.path.join(fs_root, "opengrok-proj", "opengrok")
...
def deploy(logger, url_root):
    ...
    # Removes all default web applications in tomcat
...
def run_indexer_no_projects(logger, uri, config_path, extra_indexer_options):
    """
    Project less indexer
    """
    logger.info("reindex schedule Starting")
    ...
    indexer.execute()
    logger.info("reindex schedule complete")
    logger.info("Waiting for reindex to be triggered via schedule")
...
def project_syncer(logger, loglevel, uri, config_path, numworkers, env, api_timeout):
    ...
    """
###run this syncer, it runs at startup
    wait_for_tomcat(logger, uri)
    ...
    while True:
        ...
        if os.environ.get("OPENGROK_SYNC_YML"):  # debug only
            ...
            config_file = os.path.join(fs_root, "opengrok-proj", "config", "sync.yml")
...
def get_time_from_env(logger, env_name, default_value):
    time_patterns = ["%H:%M", "%H:%M:%S"]
    env_str = os.environ.get(env_name)
    if env_str:
        for time_pattern in time_patterns:
            try:
                return str(datetime.strptime(env_str, time_pattern).time())
            except:
                logger.error("Failed to parse time {} with format {}".format(env_str, time_pattern))
                pass
    
    logger.error("{} is not a valid time format: {}".format(env_name, env_str))
    logger.debug("Using default schedule time: {}".format(default_value))
    return default_value


# Not used as there is no CHECK_INDEX variable
...
def main():
    ...
    sync_time = get_time_from_env(logger, "SYNC_TIME_STRING", "00:00")
    logger.info("Sync time: {}".format(sync_time))
    ...
    if use_projects:
        ...
        #Checks for JSON mirror file then YAML
        if not os.environ.get(NOMIRROR_ENV_NAME):
            ...
            if not check_configuration(conf):
                ...
                sync_enabled = False
        #Adds projects found in the src directory
    ...
    if sync_enabled:
        logger.debug(
            f"Creating and starting periodic sync schedule at {sync_time}"
        )
        global periodic_timer
        periodic_timer = PeriodicTimer(sync_time)
