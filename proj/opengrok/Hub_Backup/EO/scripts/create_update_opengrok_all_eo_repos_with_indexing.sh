...
create_files()
{
	BUILD_MAIL=`pwd`/All_EO_Repos_Candidate_mail.txt
        REPOSITORY_NAME_FILE=/proj/opengrok/EO-Repos/scripts/all_eo_repos.txt
...
#		continue
	...
	then
		...
		# Cloning single branch for ecm 
                if [ "$ECM_CHECK" == "ecm" ]
                then
			/proj/ciexadm200/tools/git/latest/bin/git clone --single-branch --branch development ${GERRIT_MIRROR}/$REPOSITORY
		else
			/usr/bin/git clone ${GERRIT_MIRROR}/$REPOSITORY
		fi
...
echo " "
echo "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
echo "                      Script creates/updates ALL EO GIT Repositories for OpenGrok                 "
echo "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
echo " "
...
echo "-------------------------------------------------------------------------------------------------------------------------- " > $BUILD_MAIL
echo "creates/updates ALL EO GIT Repositories for OpenGrok Jenkins Job has been Executed " >> $BUILD_MAIL
echo "-------------------------------------------------------------------------------------------------------------------------- " >> $BUILD_MAIL
echo " " >> $BUILD_MAIL
...
OPENGROK_WEBAPP_CONTEXT=/eo-opengrok OPENGROK_VERBOSE=true  OPENGROK_WEBAPP_CFGADDR="localhost:2445" OPENGROK_INSTANCE_BASE=/proj/opengrok/EO-All-Repos-index /proj/opengrok/tools/opengrok/opengrok-0.12.1.5/bin/OpenGrok  index /proj/opengrok/EO-All-Repos/src/

echo " " >> $BUILD_MAIL
echo " " >> $BUILD_MAIL
echo "Thanks and Best Regards," >> $BUILD_MAIL
echo "EO Infrastructure Team" >> $BUILD_MAIL

mailx -r "jenkins_monitoring@ericsson.com" -s "EO GIT Repositories for OpenGrok Analysis has been created/updated - ${TODAY_DATE}" $MAIL_USERS <<-EOF
`cat $BUILD_MAIL`
EOF
