...
set_environment_variables()
{
	...
	MAIL_USERS="ossrc-ciinfra@wipro.com"
}

create_files()
{
	BUILD_MAIL=`pwd`/All_OSSRC_Repos_Candidate_mail.txt
        REPOSITORY_NAME_FILE=/proj/opengrok/OSSRC-Repos/scripts/all_ossrc_repos.txt
...
get_repository_name()
{
	...
	do
		echo $REPO_LIST | egrep "^OSSRC|^OSS" >> $REPOSITORY_NAME_FILE
	done
}
...
{
	OSSRC_CHECK=`echo $REPOSITORY | awk -F"/" '{print $1}'`
	if [ "$OSSRC_CHECK" == "OSSRC" ]
	then
	...
	#cd $WORKSPACE
	cd /proj/opengrok/
	if [ ! -d OSSRC-All-Repos ]
	then
		mkdir -m 775 OSSRC-All-Repos
	fi
	cd OSSRC-All-Repos
...
echo " "
echo "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
echo "                                          Script creates/updates ALL OSSRC GIT Repositories for OpenGrok                 "
echo "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
echo " "
...
echo "-------------------------------------------------------------------------------------------------------------------------- " > $BUILD_MAIL
echo "creates/updates ALL OSSRC GIT Repositories for OpenGrok Jenkins Job has been Executed " >> $BUILD_MAIL
echo "-------------------------------------------------------------------------------------------------------------------------- " >> $BUILD_MAIL
echo " " >> $BUILD_MAIL
...
OPENGROK_WEBAPP_CONTEXT=/ossrc-opengrok OPENGROK_VERBOSE=true  OPENGROK_WEBAPP_CFGADDR="localhost:2440" OPENGROK_INSTANCE_BASE=/proj/opengrok/OSSRC-All-Repos-index /proj/opengrok/tools/opengrok/opengrok-0.12.1.5/bin/OpenGrok  index /proj/opengrok/OSSRC-All-Repos/src/

echo " " >> $BUILD_MAIL
echo " " >> $BUILD_MAIL
echo "Thanks and Best Regards," >> $BUILD_MAIL
echo "OSSRC DE Infrastructure Team" >> $BUILD_MAIL

mailx -r "jenkins_monitoring@ericsson.com" -s "OSSRC GIT Repositories for OpenGrok Analysis has been created/updated - ${TODAY_DATE}" $MAIL_USERS <<-EOF
`cat $BUILD_MAIL`
EOF
